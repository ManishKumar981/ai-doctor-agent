...
const ASSEMBLY_LANG_CODES: Record<string, string> = {
  en: 'en',
  hi: 'hi',
  te: 'en', // AssemblyAI free doesn't support te, fallback to en
}
...
const AudioProcessor = ({
  ...
  useEffect(() => {
    if (isCallActive && !audioContextRef.current) initializeAudio();
    return () => { cleanupAudio(); };
  }, [isCallActive]);

  useEffect(() => {
    if (isCallActive && !assemblySocketRef.current) connectToAssemblyAI();
  }, [isCallActive]);

  useEffect(() => {
    if (isCallActive) {
      if (isListening) { startAudioProcessing(); } else { stopAudioProcessing(); }
    }
  ...
  const connectToAssemblyAI = async () => {
    try {
      ...
      if (!tokenResponse.ok) {
        onError("AssemblyAI token fetch failed. Please check your API key.");
        return;
      }
      const { token } = await tokenResponse.json();
      if (!token) { onError("AssemblyAI API key is missing."); return; }
      ...
      // Pass language code to AssemblyAI for better transcription accuracy
      const langCode = ASSEMBLY_LANG_CODES[language] || 'en';
      const wsUrl = `wss://api.assemblyai.com/v2/realtime/ws?sample_rate=16000&token=${token}&language_code=${langCode}`;
      assemblySocketRef.current = new WebSocket(wsUrl);
      ...
      assemblySocketRef.current.onmessage = (message) => {
        ...
        if (data.message_type === 'FinalTranscript') {
          ...
          if (data.text && data.text.trim() !== "") onTranscriptReceived(data.text, false);
      ...
      assemblySocketRef.current.onclose = () => setIsAssemblyConnected(false);
    } catch {
  ...
  const startAudioProcessing = () => {
    if (!isCallActive || !isAssemblyConnected) return;
  ...
  const cleanupAudio = () => {
    ...
    if (assemblySocketRef.current) { assemblySocketRef.current.close(); assemblySocketRef.current = null; setIsAssemblyConnected(false); }
    if (microphoneStreamRef.current) { microphoneStreamRef.current.getTracks().forEach(t => t.stop()); microphoneStreamRef.current = null; }
    if (audioContextRef.current) { audioContextRef.current.close(); audioContextRef.current = null; }
    if (processorNodeRef.current) { try { processorNodeRef.current.disconnect(); } catch {} processorNodeRef.current = null; }
    if (audioSourceRef.current) { try { audioSourceRef.current.disconnect(); } catch {} audioSourceRef.current = null; }
    reconnectAttemptsRef.current = 0;
  };
